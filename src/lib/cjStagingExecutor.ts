...
export interface CjStagingDependencies {
  ...
  recordQuote(input: { attempt: number; quoteInputDigest: string; logisticName: string; logisticPriceUsd: number; fromCountryCode: string; quotedAt: number }): Promise<void>;
  stage(): Promise<{ state: "staged" | "reused" | "preflight_required"; actionId?: string }>;
...
export async function executeCjStaging(deps: CjStagingDependencies) {
  ...
  if (claimed.state === "preflight") {
    ...
    await deps.recordQuote({ attempt: claimed.attempt, quoteInputDigest: claimed.quoteInputDigest, logisticName: quote.logisticName, logisticPriceUsd: quote.logisticPriceUsd, fromCountryCode: claimed.fromCountryCode, quotedAt: deps.now() });
  }
