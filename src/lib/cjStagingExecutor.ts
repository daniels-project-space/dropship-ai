...
export interface CjStagingDependencies {
  ...
  recordQuote(input: { attempt: number; leaseGeneration: number; quoteInputDigest: string; logisticName: string; logisticPriceUsd: number; fromCountryCode: string }): Promise<void>;
  stage(): Promise<{ state: "staged" | "reused" | "preflight_required" | "needs_attention"; actionId?: string }>;
  claimApproval(): Promise<{ state: "dispatch" | "busy" | "reused" | "needs_attention"; actionId?: string; approvalDispatchKey?: string; leaseGeneration?: number; attempt?: number }>;
  ...
  recordApproval(input: { actionId: string; approvalDispatchKey: string; approvalRunId: string; leaseGeneration: number }): Promise<void>;
}
...
export async function executeCjStaging(deps: CjStagingDependencies) {
  ...
  if (claimed.state === "preflight") {
    ...
    await deps.recordQuote({ attempt: claimed.attempt, leaseGeneration: claimed.leaseGeneration, quoteInputDigest: claimed.quoteInputDigest, logisticName: quote.logisticName, logisticPriceUsd: quote.logisticPriceUsd, fromCountryCode: claimed.fromCountryCode });
  }
  ...
  await deps.recordApproval({ actionId: approval.actionId, approvalDispatchKey: approval.approvalDispatchKey, approvalRunId, leaseGeneration: approval.leaseGeneration });
