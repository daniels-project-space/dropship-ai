...
1. Generate an RSA-2048 keypair outside the repository. Configure the Vercel production environment with `DROPSHIP_AI_OPERATOR_TOKEN`, a random 32+ character `DROPSHIP_AI_SESSION_SECRET`, `DROPSHIP_AI_AUTH_PRIVATE_KEY` (PEM), `DROPSHIP_AI_AUTH_KID`, `DROPSHIP_AI_AUTH_ISSUER` (the exact public Vercel origin, without a trailing slash), `DROPSHIP_AI_AUTH_AUDIENCE` (a unique value such as `dropship-ai-production`), and `DROPSHIP_AI_SERVICE_TOKEN`.
2. Mint `DROPSHIP_AI_SERVICE_TOKEN` as an RS256 JWT using that key, with the same issuer/audience, `sub: "dropship-ai:service"`, and a short rotation period. Store it only in Vercel and the scoped Trigger project; never expose it to the browser or repository.
...
- `DROPSHIP_AI_SERVICE_TOKEN` is the sole identity accepted for Trigger/API-to-Convex calls (`sub: dropship-ai:service`). Rotate it before expiry in both Vercel and Trigger, then redeploy the Trigger workers.
