...
test("Shopify product import is structurally draft-only", async () => {
  ...
  globalThis.fetch = async (_url, options) => {
    ...
    return new Response(JSON.stringify({ data: { productCreate: { product: { id: "gid://shopify/Product/1", title: "Verified widget", handle: "verified-widget", variants: { nodes: [{ id: "gid://shopify/ProductVariant/1" }] } }, userErrors: [] } } }), { status: 200 });
  };
  try {
